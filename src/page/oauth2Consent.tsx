...
export default function OAuthAuthorizePage() {
  ...
  const handleToggleScope = (scopeId: string) => {
    setSelectedScopes((prev) => {
      ...
      newSet.has(scopeId) ? newSet.delete(scopeId) : newSet.add(scopeId)
      return newSet
    })
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 bg-gradient-to-br from-blue-50/40 via-white to-white backdrop-blur-sm">
      <Card className="w-full max-w-lg rounded-3xl border border-gray-300 bg-white/95 shadow-[0_0_20px_rgba(59,130,246,0.15)]">
            ...
            <section>
              ...
              <ul className="space-y-4 mb-12">
                {(oauthData?.previouslyApprovedScopes ?? []).map((scope) => (
                  <li key={scope.scope} className="flex items-start gap-5 p-3 rounded-lg hover:bg-green-50 transition-colors cursor-default">
                    <Badge variant="outline" className="min-w-[90px] mt-1 text-green-700 border-green-700 bg-gradient-to-tr from-green-50 to-white shadow-md">
                      {scope.scope}
                    </Badge>
                    <div>
                ...
                {scopes.map((scope) => (
                  ...
                  >
                    <div className="flex items-start gap-5 flex-1 pr-4">
                      <Badge variant="outline" className="mt-1 min-w-[90px] text-blue-700 border-blue-700 bg-gradient-to-tr from-blue-50 to-white shadow-md">
                        {scope.scope}
                      </Badge>
                      <div>
                    ...
                    <label onClick={(e) => e.stopPropagation()} className="relative flex items-center justify-center w-10 h-10 cursor-pointer">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="w-5 h-5 rounded-full border-2 border-blue-600 peer-checked:bg-blue-600 peer-checked:border-blue-600 transition-all"></span>
                      <svg
      ...
      {/* 隐藏授权表单 */}
      <form ref={authorizeFormRef} action={oauthData?.authorizationEndpoint || ""} method="POST" style={{ display: "none" }}>
      ...
      {/* 隐藏拒绝表单 */}
      <form ref={rejectFormRef} action={oauthData?.authorizationEndpoint || ""} method="POST" style={{ display: "none" }}>
        <input type="hidden" name="client_id" value={clientIdParam} />
        <input type="hidden" name="state" value={stateParam} />
      </form>
    </div>
